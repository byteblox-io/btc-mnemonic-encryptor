// BIP39 English wordlist for mnemonic auto-completion
// This is the official BIP39 English wordlist with 2048 words
...
pub fn get_bip39_suggestions(prefix: &str, limit: usize) -> Vec<String> {
    ...
    let mut suggestions: Vec<String> = BIP39_WORDLIST
        .iter()
    ...
    // Sort by length first (shorter words first), then alphabetically
...
pub fn is_valid_bip39_word(word: &str) -> bool {
    BIP39_WORDLIST.contains(&word.to_lowercase().as_str())
}
...
mod tests {
    ...
    fn test_get_bip39_suggestions() {
        let suggestions = get_bip39_suggestions("ab", 5);
        assert!(suggestions.contains(&"abandon".to_string()));
        assert!(suggestions.contains(&"ability".to_string()));
        assert!(suggestions.contains(&"able".to_string()));
        assert!(suggestions.contains(&"about".to_string()));
        assert!(suggestions.contains(&"above".to_string()));
        assert_eq!(suggestions.len(), 5);
    }

    #[test]
    fn test_get_bip39_suggestions_empty() {
        let suggestions = get_bip39_suggestions("", 5);
        assert!(suggestions.is_empty());
    }

    #[test]
    fn test_get_bip39_suggestions_no_match() {
        let suggestions = get_bip39_suggestions("xyz", 5);
        assert!(suggestions.is_empty());
    }
    ...
    fn test_is_valid_bip39_word() {
        ...
        assert!(is_valid_bip39_word("ABANDON")); // Case insensitive
        assert!(is_valid_bip39_word("Abandon"));
        assert!(!is_valid_bip39_word("notaword"));
        assert!(!is_valid_bip39_word(""));
    }

    #[test]
    fn test_suggestions_sorted() {
        let suggestions = get_bip39_suggestions("a", 10);
        ...
        for word in suggestions {
            if word.len() == prev_len {
                assert!(word >= prev_word, "Words of same length should be alphabetically sorted");
            } else {
                assert!(word.len() >= prev_len, "Words should be sorted by length first");
            }
            prev_len = word.len();
            prev_word = word;
        }
